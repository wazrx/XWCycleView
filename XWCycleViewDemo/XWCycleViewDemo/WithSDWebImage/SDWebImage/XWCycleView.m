...
@property (nonatomic, assign, getter=ishasUpdateUI) BOOL hasUpdateUI;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_titleLabelAlignment == NSTextAlignmentLeft) {
        cell.titleLabel.text = [NSString stringWithFormat:@"  %@", _titles[indexPath.item % _totleCount]];
    }else{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (!_cycleClosed) {//开启了轮播才要处理
        if (scrollView.contentOffset.x < 0) {//超出第一张的时候就移回最后一张
        ...
        if (scrollView.contentOffset.x > scrollView.frame.size.width * (_totleCount - 1)) {//超出最后一张的时候就移到第一张
